...
#pragma once
  ...
  #define LIBLET_CORE_RESTRICTHELPER_H
     ...
     * defaulting to `void` * (conventionally named "Enable"), and then use this macro in the parameter list of the
     * partial specialization, like so: template< typename T, typename Enable = void > class MyClass { ... };
     *
     *    template< typename T >
...
enum class Enabled
{
  _
     ...
     *
     * The advantage of doing it this way, with an enum rather than a generic `typename` parameter, is that by having
     * the failing substitution happen on the left-hand side of the "=", the compiler will not get confused by two
     * functions with identical signatures, but mutually exclusive constraints. Thus, the following is possible:
     *    template< typename T, MSO_ENABLE_FUNC_IF( std::is_integral< T >::value ) >
     ...
     *
     * @see MSO_ENABLE_CLASS_IF for enabling a partial specialization of a class template based on a compile-time
     * constraint
     */
    ...
    // Helper macros for constraining smart pointer conversions to mimic builtin ptr implicit conversion rules. Use
    // these with constructor, assignment operator, or cast operators that are implemented as function templates, in
    // order to cause the compiler to ignore those functions in cases where the corresponding conversion on the
    // underlying pointer type is not legal.
    //
    // In order to use these, first invoke the MSO_SMART_POINTER_TYPEDEFS function within the smart pointer's class body
    // in order to create the necessary typedefs. Then, use the appropriate MSO_IMPLICIT_CONVERSION_[FROM|TO] macro
    // within your function's template parameter list, like so:
    //
    //    template< typename MSO_IMPLICIT_CONVERSION_TO( U ) >
    //    operator U&() noexcept { return ...; }
    //
    // Use MSO_IMPLICIT_CONVERSION_TO when implementing a cast operator, and MSO_IMPLICIT_CONVERSION_FROM when
    // implementing a converting constructor or assignment operator.
